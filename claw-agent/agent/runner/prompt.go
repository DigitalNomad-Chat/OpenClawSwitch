...
// clawHome is the anna home directory (e.g. ~/.claw).
func BuildSystemPrompt(store *memory.Store, clawHome, workspace string, cwd ...string) string {
	workDir := ""
	if len(cwd) > 0 {
		workDir = cwd[0]
	}
	...
	if skills := FormatSkillsForPrompt(LoadSkills(clawHome, workspace, workDir)); skills != "" {
		buf.WriteString("\n")
