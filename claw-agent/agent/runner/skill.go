...
import (
	...
	"claw-agent/agent/runner/builtin"
	"gopkg.in/yaml.v3"
)
...
// clawHome is the anna home directory (e.g. ~/.claw), workspace is the workspace
// dir (e.g. ~/.claw/workspace), cwd is the working directory.
// Priority order: cwd/.agents/skills/ > workspace/skills/ > ~/.agents/skills/ > builtin
func LoadSkills(clawHome, workspace, cwd string) []Skill {
	home, _ := os.UserHomeDir()
	return loadSkills(home, clawHome, workspace, cwd)
}

func loadSkills(homeDir, clawHome, workspace, cwd string) []Skill {
	...
	// 2. Workspace skills: workspace/skills/ (e.g. ~/.claw/workspace/skills/)
	if workspace != "" {
	...
	if clawHome != "" {
		builtinDir := filepath.Join(clawHome, "cache", "builtin-skills")
