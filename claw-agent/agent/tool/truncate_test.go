...
func TestTruncateHeadByLines(t *testing.T) {
	t.Setenv("CLAW_TOOL_MAX_LINES", "3")
	t.Setenv("CLAW_TOOL_MAX_BYTES", "999999")
...
func TestTruncateHeadByBytes(t *testing.T) {
	t.Setenv("CLAW_TOOL_MAX_LINES", "999999")
	t.Setenv("CLAW_TOOL_MAX_BYTES", "20")
...
func TestTruncateHeadExactThreshold(t *testing.T) {
	t.Setenv("CLAW_TOOL_MAX_LINES", "5")
	t.Setenv("CLAW_TOOL_MAX_BYTES", "999999")
...
func TestMaxLinesEnvOverride(t *testing.T) {
	t.Setenv("CLAW_TOOL_MAX_LINES", "500")
	if n := maxLines(); n != 500 {
...
func TestMaxLinesEnvInvalid(t *testing.T) {
	t.Setenv("CLAW_TOOL_MAX_LINES", "bad")
	if n := maxLines(); n != defaultMaxLines {
...
func TestMaxBytesEnvOverride(t *testing.T) {
	t.Setenv("CLAW_TOOL_MAX_BYTES", "1024")
...
func TestReadToolPaginationAdvancesOnZeroOutputLines(t *testing.T) {
	...
	t.Setenv("CLAW_TOOL_MAX_LINES", "999999")
	t.Setenv("CLAW_TOOL_MAX_BYTES", "10") // Very small byte limit
...
func verifyTempFileInContent(t *testing.T, content, expectedContent string) {
	t.Helper()
	// Format: [Full output saved to /tmp/claw-tool-xxx.txt — use the read tool to access]
	idx := strings.Index(content, "/tmp/claw-tool-")
	if idx < 0 {
